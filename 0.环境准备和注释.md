# 内容介绍
- 在Windows、Mac、Linux**安装Python环境**的方式
- Python**交互式环境**的使用方法和特点
- Python**命令行模式**的使用方法和特点
- 推荐使用**文本编辑器**或者说**集成开发环境**(IDE): Visual Studio Code
- Python**注释**的写法
...
Mac的话类似Windows；如果你用的是Linux，那假定你有Linux系统管理经验，自行安装Python 3肯定没有问题，否则，请换回Windows系统。

## 交互式模式

>注：Python在Linux/macOS的命令是`python3`，在Windows下的命令是`python`，后续请自行根据操作系统选择合适的命令。

安装成功后，Windows下打开**PowerShell**或者**命令提示符**，Win+S快捷键打开Windows开始菜单搜索Search功能，搜索**PowerShell**或者**命令提示符**即可打开。
...
![print-explain](https://liaoxuefeng.com/books/python/first-program/input-output/print.jpg)
